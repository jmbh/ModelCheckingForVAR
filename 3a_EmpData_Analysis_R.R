...
# Simulate from estimated model using mlVAR::resimulate
sim_out <- resimulate(mlVAR_out,
                      keep_missing = TRUE, # reproduce empirical missingness structure
                      variance = "empirical") # inform residual variance empirically

# Get this into list format, which we use below for the figures
l_PPCs <- list()
for(i in 1:n_ptp) l_PPCs[[i]] <- list(data_sim = sim_out[sim_out$id == u_ptp[i], 4:7])
...
for(i in 1:3) {
  pdf(paste0("Figures/mlVAR/Fig_EmpAnalysis_Diagnostics_", sel[i], ".pdf"), width=11, height=9.5)
...
pdf("Figures/mlVAR/Fig_EmpAnalysis_Diagnositics_All.pdf", width=11, height=9.5)
for(i in 1:n_ptp) {
  print(PlotDiagnosticsEmp(l_ResObj = l_ResObj, 
                           l_PPC = l_PPCs,  
                           subject = i, 
...
pdf("Figures/mlVAR/Fig_EmpAnalysis_R2_RMSE_agg_ggplot.pdf", width = 8, height = 4)
...
pdf("Figures/mlVAR/Fig_EmpAnalysis_R2_RMSE_comp_ggplot.pdf", width=8, height=8)
...
sc <- 2
pdf("Figures/mlVAR/Fig_EmpAnalysis_phi_and_REvar.pdf", width = 2*6*sc, height=6*sc)
...
## Some numbers reported in the paper
# Diagonal
...
# Arrange in Layout
sc <- 0.95
pdf("Figures/mlVAR/Fig_mlVAR_RE_Distr_phi_ggplot.pdf", width=12*sc, height=11*sc)
...
sc <- 0.95
pdf("Figures/mlVAR/Fig_mlVAR_RE_Distr_intcps_ggplot.pdf", width=12*sc, height=3.5*sc)
